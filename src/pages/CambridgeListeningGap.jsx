...
export default function CambridgeListeningGap() {
  ...
  function submitCloze() { setClozeSubmitted(true) }
    ...
    <CambridgeLayout activeModule="listening" level={level} setLevel={setLevel}>
      ...
      <main className="mx-auto max-w-6xl px-6 py-7 lg:px-8">
        ...
        <section className="mt-6 rounded-[22px] border border-gray-200 bg-white p-4 shadow-sm">
          ...
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-5 lg:grid-cols-8">{LISTENING_GAP_EXERCISES.map(item => <button key={item.exerciseNumber} onClick={() => setExerciseNumber(item.exerciseNumber)} className={`rounded-xl border px-3 py-2.5 text-sm font-extrabold transition ${exerciseNumber === item.exerciseNumber ? 'border-[#e4b83f] bg-[#f7cd60] text-[#4c3a00] shadow-sm' : 'border-gray-200 bg-white text-gray-600 hover:border-[#e4b83f] hover:bg-[#fffaf0]'}`}>{item.label}</button>)}</div>
        </section>
        ...
        {current.cloze ? <section className="mt-5 overflow-hidden rounded-[24px] border border-gray-200 bg-white shadow-sm">
          ...
          <div className="border-t border-gray-100 p-5"><button onClick={submitCloze} disabled={clozeAnswers.some(item => !(clozeResponses[item.key] || '').trim())} className="w-full rounded-xl bg-[#064e3b] px-5 py-4 text-lg font-extrabold text-white transition hover:bg-[#07634b] disabled:cursor-not-allowed disabled:bg-gray-200 disabled:text-gray-400">提交全部填空</button>{clozeSubmitted && <div className="mt-3 rounded-xl bg-[#fff8e7] px-4 py-3 text-center"><strong className="text-xl text-[#735500]">{clozeAnswers.filter(clozeIsCorrect).length} / {clozeAnswers.length}</strong><div className="mt-0.5 text-xs font-bold text-[#8a6a19]">本组填写正确的空格</div></div>}</div>
